...
contract HouseSession {
    ...
    constructor(address _token, address _custody) {
        if (_token == address(0) || _custody == address(0))
            revert ZeroAddress();
        token = _token;
    ...
    /// @dev One active session per player. sessionHash = sha256(seed, player)
