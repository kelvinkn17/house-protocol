...
function BuildLayout() {
  ...
  return (
    <AppLayout noPadding>
      <div className="min-h-screen bg-[#EDEBE6] flex">
        {/* sidebar */}
        <aside
          ref={sidebarRef}
...
const RECENT_GAMES = [
  { id: 'lucky-flip', name: 'Lucky Flip', type: 'pick-one', status: 'active', earnings24h: 62, volume24h: 12400 },
  { id: 'dice-royale', name: 'Dice Royale', type: 'pick-number', status: 'active', earnings24h: 30.75, volume24h: 8200 },
  { id: 'moon-crash', name: 'Moon Crash', type: 'cash-out', status: 'pending', earnings24h: 0, volume24h: 0 },
]
    ...
    <div ref={containerRef} className="p-4 md:p-6 lg:p-8 max-w-5xl">
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-5 pb-5 border-b-2 border-black/10">
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-black tracking-tight">Welcome back</h1>
            <p className="text-black/50 font-mono text-xs mt-1">** Build games, earn 25% of house edge</p>
          </div>
          <Link
            to="/build/games/new"
      ...
      <div className="grid lg:grid-cols-5 gap-5">
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={cnm(
                      ...
                      game.status === 'active' ? 'bg-[#CDFF57] text-black' : 'bg-black/10 text-black/40'
                    )}
                  >
                    {game.name.charAt(0)}
                  </div>
                  <div>
                    ...
                    <p className="text-[10px] font-mono text-black/50">{game.type} • {game.status}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="text-right hidden sm:block">
                    <p className="text-[10px] font-mono text-black/40">Vol</p>
                    <p className="font-bold text-black text-sm">${game.volume24h.toLocaleString()}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-mono text-black/40">Earned</p>
        ...
        >
          ...
          <div className="p-4 bg-black">
            <p className="text-xs font-mono text-white/50 uppercase mb-2">Revenue Split</p>
            <p className="text-[10px] font-mono text-white/40 mb-3">$100 bet @ 2% edge = $2</p>
